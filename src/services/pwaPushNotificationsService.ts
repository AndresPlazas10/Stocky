...
export async function getWebPushSupportStatus() {
  ...
  if (!('serviceWorker' in navigator)) {
    return { supported: false, reason: 'missing_service_worker', permission: normalizePermission((window as Window & typeof globalThis).Notification?.permission) };
  }

  if (!('PushManager' in window)) {
    return { supported: false, reason: 'missing_push_manager', permission: normalizePermission((window as Window & typeof globalThis).Notification?.permission) };
  }
