...
function buildSalesListCacheKey({ businessId, filters = {} as SalesFilters, pagination = {} as SalesPagination }: { businessId: string; filters?: SalesFilters; pagination?: SalesPagination }) {
  return [
    'sales',
    'list',
    businessId,
...
async function readCachedSalesList(_cacheKey: string): Promise<Record<string, unknown> | null> {
  return null;
}

async function writeCachedSalesList(_cacheKey: string, _payload: unknown): Promise<void> {
}
...
function mapRpcSalesRows(rows: Record<string, unknown>[] = [], includeCount = true) {
  const data = rows.map((row) => {
    const employeeRole = row.employee_role || 'employee';
    const isOwner = !!row.is_owner;
    const isAdmin = isAdminRole(employeeRole as string);
...
export async function getSales(businessId: string) {
  try {
    ...
    (employees as unknown as Record<string, unknown>[])?.forEach(emp => {
      if (emp.user_id) {
        employeeMap.set(emp.user_id as string, {
    ...
    const enrichedSales = ((sales as unknown as Record<string, unknown>[]) || []).map(sale => {
      const userId = sale.user_id;
      const employee = userId ? employeeMap.get(userId as string) : null;
      
      const isOwner = userId && (business as unknown as Record<string, unknown>)?.created_by && 
                      String(userId).trim() === String((business as unknown as Record<string, unknown>).created_by).trim();
      
      // Determinar si es admin
      const isAdmin = isAdminRole(employee?.role as string);
      ...
      return {
        ...
        employees: isOwner
          ...
          ? { full_name: 'Administrador', role: 'admin' }
          : employee || { 
              full_name: (sale as Record<string, unknown>).seller_name || 'Empleado', 
              role: 'employee' 
            }
      };
    });
 ...
 */
export async function getFilteredSales(businessId: string, filters: SalesFilters = {}, pagination: SalesPagination = {}) {
  ...
  try {
    if (!salesRpcDisabled) {
      ...
      if (rpcError) {
        if (isMissingRpcError(rpcError as unknown as Record<string, unknown>, 'get_sales_enriched') || isRpcBadRequestError(rpcError as unknown as Record<string, unknown>)) {
          salesRpcDisabled = true;
          logger.warn('[sales-service] disabling get_sales_enriched RPC fallback after remote error', {
            code: (rpcError as unknown as Record<string, unknown>)?.code || null,
            status: (rpcError as unknown as Record<string, unknown>)?.status || (rpcError as unknown as Record<string, unknown>)?.statusCode || null,
            message: (rpcError as unknown as Record<string, unknown>)?.message || String(rpcError)
          });
    ...
    try {
      ...
      const enriched = ((sales as unknown as Record<string, unknown>[]) || []).map(sale => {
        const employee = sale.user_id ? employeeMap.get(sale.user_id as string) : null;
        const isOwner = sale.user_id && (business as unknown as Record<string, unknown>)?.created_by &&
                        String(sale.user_id).trim() === String((business as unknown as Record<string, unknown>).created_by).trim();
        const isAdmin = isAdminRole(employee?.role as string);
...
export async function createSale({
  ...
  businessId: string;
  cart: Array<{ product_id?: string; combo_id?: string; quantity: number; unit_price: number }>;
  ...
  try {
    ...
    const isOwner = user.id && (business as unknown as Record<string, unknown>)?.created_by && String(user.id).trim() === String((business as unknown as Record<string, unknown>).created_by).trim();
    const isAdmin = isAdminRole((employee as unknown as Record<string, unknown>)?.role as string);

    const sellerName = isOwner || isAdmin
      ? 'Administrador'
      : (((employee as unknown as Record<string, unknown>)?.full_name || user.email || 'Vendedor') as string);
...
export async function deleteSale(saleId: string) {
  try {
    ...
    if (details && (details as unknown as Record<string, unknown>[]).length > 0) {
      const productUpdates = (details as unknown as Record<string, unknown>[])
