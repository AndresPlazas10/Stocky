...
export default function PerformanceHud({
  ...
  const [snapshots, setSnapshots] = useState<
    Array<Record<string, unknown> & { section?: string }>
  >([]);
  ...
  useEffect(() => {
    ...
    setSnapshots((prev) =>
      [...prev, snapshot].slice(-MAX_EXPORTED_SNAPSHOTS)
    );
  ...
  const summary = useMemo(() => {
    ...
    return {
      ...
      avgFps: round(average(snapshots.map((s) => s.fps as number)), 1),
      avgFrameMs: round(
        average(snapshots.map((s) => s.avgFrameMs as number)),
        2
      ),
      avgP95FrameMs: round(
        average(snapshots.map((s) => s.p95FrameMs as number)),
        2
      ),
      avgSlowFrames60Pct: round(
        average(snapshots.map((s) => s.slowFrames60Pct as number)),
        1
      ),
      avgSlowFrames30Pct: round(
        average(snapshots.map((s) => s.slowFrames30Pct as number)),
        1
      ),
      totalLongTasks: snapshots.reduce(
        (sum, s) => sum + Number(s.longTasks || 0),
        0
      ),
      totalLongTaskTimeMs: round(
        snapshots.reduce(
          (sum, s) => sum + Number(s.longTaskTimeMs || 0),
          0
        ),
        1
      ),
    ...
    <div className="fixed bottom-4 right-4 z-[120] w-[320px] max-w-[calc(100vw-1.5rem)] rounded-2xl border border-slate-200 bg-white/95 p-3 shadow-2xl backdrop-blur">
      ...
      <div className="grid grid-cols-2 gap-2 text-xs">
        ...
        <div className="rounded-lg border border-slate-200 bg-slate-50 p-2">
          ...
          <p className={`font-bold ${getFpsTone(stats.fps)}`}>
            {stats.fps}
          </p>
        </div>
        <div className="rounded-lg border border-slate-200 bg-slate-50 p-2">
        ...
        <div className="rounded-lg border border-slate-200 bg-slate-50 p-2">
          ...
          <p className="font-semibold text-slate-900">
            {stats.slowFrames60Pct}%
          </p>
        </div>
        <div className="rounded-lg border border-slate-200 bg-slate-50 p-2">
          <p className="text-slate-500">&gt;33.3 ms</p>
          <p className="font-semibold text-slate-900">
            {stats.slowFrames30Pct}%
          </p>
        </div>
        <div className="rounded-lg border border-slate-200 bg-slate-50 p-2">
        ...
        <div className="rounded-lg border border-slate-200 bg-slate-50 p-2">
          ...
          <p className="font-semibold text-slate-900">
            {stats.longTaskTimeMs} ms
          </p>
        </div>
      </div>

      <p className="mt-2 text-[11px] text-slate-500">
        Muestra: {stats.sampleFrames} frames en{" "}
        {stats.sampleWindowMs || sampleWindowMs} ms
      </p>
      <p className="text-[11px] text-slate-500">
        Sesión: {snapshots.length} snapshots • avg FPS{" "}
        {summary.avgFps} • avg p95 {summary.avgP95FrameMs} ms
      </p>
      <p className="text-[11px] text-slate-500">
