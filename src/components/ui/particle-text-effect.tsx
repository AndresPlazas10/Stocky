...
class Particle {
  ...
  particleSize = 10;
  isKilled = false;
  startColor: ColorRGB = { r: 0, g: 0, b: 0 };
...
function generateRandomPos(x: number, y: number, mag: number): Vec2 {
...
export function ParticleTextEffect({
  ...
  useEffect(() => {
    ...
    const nextWord = (word: string) => {
      ...
      const fontSize = Math.max(40, canvas.width / 8);
  ...
  if (prefersReducedMotion) {
    return (
      <div className="flex items-center justify-center py-8">
        <h1 className="text-4xl sm:text-5xl lg:text-6xl font-bold tracking-tight text-primary sm:text-6xl">
