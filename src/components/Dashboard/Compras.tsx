...
function Compras({ businessId }: DashboardModuleProps) {
  ...
  useRealtimeSubscription('purchases', {
    ...
    onDelete: (deletedPurchase) => {
      setPurchases(prev => prev.filter(c => c.id !== deletedPurchase.id));
    }
  });
  ...
  const filteredPurchases = useMemo(() => {
    ...
    return purchases.filter(purchase =>
      purchase.supplier?.business_name?.toLowerCase().includes(search) ||
      purchase.supplier?.contact_name?.toLowerCase().includes(search) ||
      purchase.payment_method?.toLowerCase().includes(search)
    );
  ...
  return (
    <AsyncStateWrapper
      loading={loading}
      error={filteredPurchases.length === 0 ? error : null}
      dataCount={filteredPurchases.length}
                  ...
                  <div className="p-4 space-y-3">
                    ...
                    {purchase.notes && (
                      <div className="pt-2 border-t border-gray-100">
                        <p className="text-xs text-gray-500 italic">"{purchase.notes}"</p>
                      </div>
                    )}
