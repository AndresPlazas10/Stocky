...
export interface FramePerformanceStats {
  ...
  avgFrameMs: number;
  p95FrameMs: number;
  slowFrames60Pct: number;
  slowFrames30Pct: number;
  sampleFrames: number;
...
const emptyStats: FramePerformanceStats = {
  fps: 0,
  avgFrameMs: 0,
  p95FrameMs: 0,
  slowFrames60Pct: 0,
...
const percentile = (values: number[] = [], p = 0.95): number => {
...
interface UseFramePerformanceOptions {
  enabled?: boolean;
  sampleWindowMs?: number;
}
...
}: UseFramePerformanceOptions = {}): FramePerformanceStats {
  const [stats, setStats] = useState<FramePerformanceStats>(emptyStats);
