...
export function buildKitchenEscPos(
  ...
  bold(out, true);
  size(out, true);
  writeLine(out, cleanText(receipt.header?.title || 'ORDEN DE COCINA'));
  size(out, false);
  bold(out, false);
  feed(out, 1);

  const dateText = cleanText(receipt.header?.dateText || '');
  if (dateText) {
    writeLine(out, dateText);
    feed(out, 1);
  }
  ...
  bold(out, true);
  twoColumns(out, cleanText(receipt.productHeader || 'Producto'), cleanText(receipt.quantityAbbreviation || 'Cant.'), columns);
  bold(out, false);
  feed(out, 1);
  ...
  if (items) {
    for (const item of items) {
      if (!item) continue;
      bold(out, true);
      twoColumns(out, cleanText(item.name || 'Item'), 'x' + (item.quantity || 0), columns);
      bold(out, false);
      feed(out, 1);
    }
  }
  ...
  align(out, 1);
  bold(out, true);
  writeLine(out, cleanText(receipt.footer?.message || '*** ORDEN PARA COCINA ***'));
  bold(out, false);
