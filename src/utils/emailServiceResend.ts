    ...
    const _htmlContent = `
      ...
      <body style="margin: 0; padding: 0; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; background-color: #f5f5f5;">
        <table width="100%" cellpadding="0" cellspacing="0" style="max-width: 600px; margin: 0 auto; background-color: #ffffff;">
          ...
          <!-- Header -->
          <tr>
            <td style="background: linear-gradient(135deg, #66A5AD 0%, #99D3DB 100%); padding: 40px 20px; text-align: center;">
            ...
            <td style="padding: 30px 20px;">
              ...
              <table width="100%" style="margin: 20px 0; background-color: #f9f9f9; border-radius: 8px; padding: 15px;">
                ...
                <tr>
                  ...
                  <td style="padding: 5px; text-align: right; color: #66A5AD; font-size: 20px; font-weight: bold;">$${total.toLocaleString('es-CO')}</td>
                </tr>
              </table>

              <!-- Items Table -->
              <h2 style="color: #66A5AD; font-size: 18px; margin: 30px 0 15px 0;">Detalle de Productos</h2>
              <table width="100%" style="border-collapse: collapse;">
                <thead>
                  <tr style="background-color: #66A5AD; color: white;">
 ...
 */
export const isResendConfigured = (env?: Partial<ImportMetaEnv>) => {
  const providerHint = String(env?.VITE_EMAIL_PROVIDER || import.meta.env?.VITE_EMAIL_PROVIDER || 'auto').trim().toLowerCase();
  if (providerHint === 'emailjs') return false;

  const explicitDisable = String(env?.VITE_RESEND_ENABLED || import.meta.env?.VITE_RESEND_ENABLED || '').trim().toLowerCase();
