...
type PaymentMethod = 'cash' | 'card' | 'transfer' | 'mixed' | 'nequi' | 'bancolombia' | 'banco_bogota' | 'nu' | 'davivienda';
...
export function getPaymentMethodLabel(method: string | null | undefined, options?: { emoji?: boolean }): string {
  ...
  if (normalized === 'banco_bogota') return options?.emoji ? '🏦 Banco de Bogotá' : 'Banco de Bogotá';
...
export function getPaymentMethodTheme(method: string | null | undefined): {
  ...
  const normalized = normalizeMethod(method);
  if (normalized === 'card') {
    return { icon: 'card-outline', backgroundColor: '#DBEAFE', textColor: '#1D4ED8', iconColor: '#2563EB' };
  }
  if (normalized === 'transfer') {
    return { icon: 'swap-horizontal-outline', backgroundColor: '#E0E7FF', textColor: '#4338CA', iconColor: '#4F46E5' };
  }
  if (normalized === 'mixed') {
    return { icon: 'layers-outline', backgroundColor: '#F3E8FF', textColor: '#7E22CE', iconColor: '#9333EA' };
  }
  if (BANK_METHODS.has(normalized)) {
    return { icon: 'business-outline', backgroundColor: '#F3E8FF', textColor: '#7E22CE', iconColor: '#9333EA' };
  }
  return { icon: 'cash-outline', backgroundColor: '#DCFCE7', textColor: '#166534', iconColor: '#16A34A' };
}

export function getPaymentMethodIcon(method: string | null | undefined): keyof typeof Ionicons.glyphMap {
  const normalized = normalizeMethod(method);
