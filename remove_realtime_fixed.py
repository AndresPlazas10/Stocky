#!/usr/bin/env python3
"""
Script corregido para eliminar funciones realtime inline
"""
...
def main():
    ...
    for func_name in function_names:
        ...
        # Encontrar el cierre (buscar línea que termine con ); o ], []); etc)
        bracket_count = 0
        end_idx = None
        ...
        for i in range(start_idx, len(lines)):
            ...
            # Verificar si es el cierre de useCallback
            if bracket_count == 0 and re.search(r'\}\s*,\s*\[.*\]\s*\)\s*;', line):
                end_idx = i
                break
        
        if end_idx is not None:
            ranges_to_remove.append((start_idx, end_idx, func_name))
            print(f"✓ {func_name}: líneas {start_idx+1}-{end_idx+1} ({end_idx - start_idx + 1} líneas)")
        else:
