-- ============================================================
-- Corregir validación de manage_stock en ventas con combos
-- Fecha: 2026-07-16
-- Problema:
--   La migración 20260715_0100 restauró soporte de combos pero
--   perdió la validación del campo manage_stock de productos.
--   Esto causa que productos con manage_stock = false (ej: mano
--   de obra, servicios) sean rechazados por "stock insuficiente"
--   cuando se venden dentro de un combo.
-- Solución:
--   Reescribir create_sale_complete para:
--   1. Consultar manage_stock al insertar productos de combo
--   2. Validar stock solo si manage_stock = true
--   3. Descontar stock solo si manage_stock = true
-- ============================================================
...
DECLARE
  ...
  v_product_manage_stock boolean;
  v_combo_name text;
...
BEGIN
  ...
  LOOP
    ...
    IF v_product_id IS NOT NULL THEN
      SELECT p.name, p.sale_price, p.manage_stock
      INTO v_product_name, v_product_sale_price, v_product_manage_stock
      ...
      INSERT INTO tmp_stock_updates (product_id, qty_total, requires_stock_check, manage_stock)
      VALUES (
        v_product_id,
        v_quantity,
        COALESCE(v_product_manage_stock, true),
        COALESCE(v_product_manage_stock, true)
      )
    ...
    SELECT
      ...
      COALESCE(p.manage_stock, true),
      COALESCE(p.manage_stock, true)
    FROM public.combo_items ci
    JOIN public.products p ON p.id = ci.producto_id
    WHERE ci.combo_id = v_combo_id
    ON CONFLICT (product_id)
    DO UPDATE SET
      qty_total = tmp_stock_updates.qty_total + EXCLUDED.qty_total,
  ...
  LOOP
    ...
    IF COALESCE(v_stock_row.manage_stock, true)
       AND v_stock_row.requires_stock_check
  ...
  -- Descuento de inventario consolidado (solo productos con manage_stock = true).
  ...
  RETURN QUERY SELECT v_sale_id, v_total, v_item_count, 'success'::text;
END;
$$;
...
IS 'Crea una venta con soporte de combos. Respeta manage_stock: productos con manage_stock=false no validan ni descuentan stock.';
