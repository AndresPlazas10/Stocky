-- Fix crítico: list_open_order_snapshot_fast NO embebía los nombres de
-- productos/combos en el jsonb `items` (a diferencia del RPC legacy
-- list_open_order_snapshot). Resultado: la cocina móvil mostraba "Item"
-- para todos los productos.
--
-- Se re-crea la función con LEFT JOIN a products/combos (lookup por PK
-- indexado, costo mínimo) y el jsonb `products`/`combos` embebido, espejo
-- del RPC legacy. La firma no cambia: CREATE OR REPLACE es suficiente.
...
IS 'Snapshot optimizado de orden abierta para mobile, con nombres de productos/combos embebidos.';
