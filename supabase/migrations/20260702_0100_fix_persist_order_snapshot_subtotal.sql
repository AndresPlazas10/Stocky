...
-- Fix: order_items.subtotal is a GENERATED ALWAYS AS (quantity * price) STORED column.
-- PostgreSQL forbids explicit UPDATE of generated columns (error 428C9).
-- Remove subtotal from UPDATE SET and INSERT; PostgreSQL auto-computes it.
...
BEGIN
  ...
  -- UPDATE: only set quantity and price; subtotal is auto-computed by PostgreSQL
  ...
  WHERE oi.order_id = p_order_id
    AND (
      (src.product_id IS NOT NULL AND oi.product_id = src.product_id AND oi.combo_id = NULL)
      OR
      (src.combo_id IS NOT NULL AND oi.combo_id = src.combo_id AND oi.product_id = NULL)
    );

  -- INSERT: only provide quantity and price; subtotal is auto-computed
...
IS 'Persiste atomicamente el snapshot de order_items. Corregido: no intentaUPDATE subtotal (es generated stored column).';
