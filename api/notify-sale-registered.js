...
import { validateBody, NotifySaleSchema } from './_lib/validation.js';
...
export default async function handler(req, res) {
  ...
  try {
    const validation = validateBody(NotifySaleSchema, req, res);
    ...
    const { business_id, sale_total } = validation.data;
    const businessId = normalizeText(business_id);
    const saleTotal = normalizeNumber(sale_total, null);
    ...
    const messages = uniqueExpoTokens.map((pushToken) => ({
      ...
      body: `Se registro una nueva venta por ${totalLabel}.`,
      data: {
